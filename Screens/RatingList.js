...
// import { Canvas, Fill, BackdropBlur, ColorMatrix, useImage } from "@shopify/react-native-skia";
...
function RatingList(props) {
  ...
  const loadData = () => {
    ...
    setLoading(true)
    fetch(`${BASE_URL}/api/rating/?gender=${gender}&limit=20` + (cursorId != 0 && currentPage != 1 ? `&cursor_id=${cursorId}&cursor_rating=${cursorRating}` : ''), {
      method:"GET",
      headers: {
  ...
  useEffect(() => {
    props.navigation.setOptions({
      title: language == 'English' ? 'Staff' : 'Персонал', headerTitleStyle: {fontFamily: 'SftBold', fontSize: 20},
      headerRight: () => (
        <TouchableOpacity
  ...
  const renderData = (item) => {
    ...
    <TouchableOpacity style={styles.cardStyle} activeOpacity = {1} onPress = {() => clickedItem(item)}>
      ...
      {item.verified && <CheckMark width={35} height={35} style={{ position: 'absolute', zIndex: 1, right: -5, top: -5 }} />}
      <ImageBackground
