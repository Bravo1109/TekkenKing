...
function Profile(props) {
  ...
  const {swipes} = props.route.params.swipes != undefined ? props.route.params.swipes : false
  let onScrollEnd = (e) => {
  ...
  const openNewChat = () => {
    let status = 0
    fetch(`${BASE_URL}/api/users/${userData.id}/messagepay/`, {
      method:"GET",
      headers: {
        'Authorization': `${token._j}`
      }
  ...
  return (
    <View style={{ flex:1, backgroundColor: '#172136', width: '100%'}}>
      ...
      <TouchableOpacity
            onPress={() => {
              if (swipes == true) {
                props.navigation.navigate('SwipesPage', { likedChanged, id });
              } else {
          ...
          <TouchableOpacity
            onPress={() => {
              userData.profile && userData.profile.my_profile == 0 && !userData.deleted && !userData.blocked && setModalEmoji(true)
            }}
            style={{borderRadius: 100, width: 40, height: 40,
            ...
            activeOpacity={1}
            >
              <SendReaction width={30} height={30} fill={userData.profile && userData.profile.my_profile == 0 && !userData.deleted && !userData.blocked ? '#2688EB' : '#aaa'} />
          </TouchableOpacity>
          </View>
      <View style={{ width: '100%', height: 90, backgroundColor: '#0F1421',
        position: 'absolute', bottom: 0, zIndex: 100, alignItems: 'center' }}>
        <View style={{ width: '90%', marginTop: 10 }}>
        {userData.profile && userData.profile.my_profile == 0 && !userData.deleted && !userData.blocked ? <TouchableOpacity
          onPress={() => {
            if (userData.profile.match == 1 || userData.is_my_premium) {
              setModal(true)
            } else {
              setModalCash(true)
              console.log('nooooo')
    ...
    showsVerticalScrollIndicator={false}
     >
      <View style={{width:'100%', height: 0.75 * SCREEN_HEIGHT}}>
        {!userData.deleted && !userData.blocked ? <ImageBackground
        source={
        ...
        <View
          ...
          />}
            ...
            {Platform.OS === 'android' && !userData.deleted && !userData.blocked && <LinearGradient
            ...
            />}
              ...
              <View
                style={{ flexDirection: 'row', alignItems: 'center' }}
              >
               {!userData.deleted && !userData.blocked ? <Text style={{fontSize: 25, textAlign: 'left', marginLeft: 0, fontFamily: 'SftBold',
              ...
              >{!userData.name.length <= 20
                ...
                color: '#fff', letterSpacing: -1}}>{userData.deleted ? 'DELETED' : 'BLOCKED'}</Text>}
                ...
                {userData.profile && (userData.profile.liked == 0 || userData.profile.liked == 2) && !userData.deleted && !userData.blocked ? <TouchableOpacity
                  style={{ marginRight: 20 }}
                  activeOpacity={1}
                  onPress={() => {
                    animateColor(1)
                    {userData.profile.liked != 1 && userData.profile.liked != 2 && sendDislike()}
                  }}
                >
                ...
                ><Svg xml={Close} width="55" height="55" viewBox="27 27 50 50">
                  ...
                  {userData.profile && (userData.profile.liked == 0 || userData.profile.liked == 1) && !userData.deleted && !userData.blocked  ? <TouchableOpacity
                  activeOpacity={1}
                  onPress={() => {
                    animateColor(0)
                    {userData.profile.liked != 1 && userData.profile.liked != 2 && sendLike(userData.tokens)}
                  }}
        ...
        >
          ...
          <View style={{justifyContent: 'center', alignItems: 'center', width: '33%'}}>
            ...
            {userData.profile && <Text style={{fontSize:20, color: '#fff', fontFamily: 'SftBold'}}>{userData.profile.comunication < 10 ? userData.profile.comunication.toFixed(1) : userData.profile.comunication}/10</Text>}
          </View>
          <View style={{justifyContent: 'center', alignItems: 'center', width: '33%'}}>
            <Text
              numberOfLines={1}
            ...
            {userData.profile && <Text style={{fontSize:20, color: '#fff', fontFamily: 'SftBold'}}>{userData.profile.activity < 10 ? userData.profile.activity.toFixed(1) : userData.profile.activity}/10</Text>}
          </View>
          </TouchableOpacity>
        ...
        {!userData.deleted && !userData.blocked && <View style={{ width: '90%', marginBottom: 100, paddingHorizontal: 5 }}>
        ...
        marginTop: 20, marginBottom: 8}]}>{language == 'English' ?  'About me' : 'О себе'}</Text>}
          {userData.profile && userData.profile.description.length > 0 && <Text
           ...
           numberOfLines={12}
          >{userData.profile.description}</Text>}
        {userData.profile && userData.profile.tags.length > 0 && <Text style={[{fontSize:30, width: '100%', fontWeight: '600', color: '#fff', shadowColor: '#fff', fontFamily: 'SftBold',
        // shadowRadius: 10, shadowOpacity: 1, shadowOffset: {width: 0, height: 0},
        ...
        // shadowRadius: 10, shadowOpacity: 1, shadowOffset: {width: 0, height: 0},
         marginTop: 15, marginBottom: 12, fontFamily: 'SftBold'}]}>{language == 'English' ?'Info' : 'Информация'}</Text>
    ...
    >
        <View style={{
          ...
          />
            <Text
              style={{ textAlign: 'center', fontSize: 25, color: '#fff', fontFamily: 'SftBold', width: '80%' }}
            >{language == 'English' ? 'No match' : 'Нет мэтча'}</Text>
            <Text
              style={{ textAlign: 'center', fontSize: 17, color: '#818C99', fontFamily: 'SftLight',
               width: '80%', marginTop: 10 }}
            >{language == 'English' ? 'You don’t have a match with this user. Do you want to open this chat?' : 'У вас нет взаимной симпатии с этим пользователем. Хотите разблокировать чат?'}</Text>
            <Image
              source={require('../images/chatOpen.png')}
              style={{ width: '50%', aspectRatio: 1/1, overflow: 'visible', marginVertical: 10 }}
            />
    ...
    >
        <View style={{
          ...
          />
            <Text
                style={{ textAlign: 'center', fontSize: 25, color: '#fff', fontFamily: 'SftMedium', marginTop: 30 }}
            ...
            <View style={{
              ...
              >{errText}</Text>
              <Image source={require('../images/dollar.png')} style={{ width: 20, aspectRatio: 1/1, marginLeft: 5 }} />
            </View>
