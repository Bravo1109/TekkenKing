...
const RateSlider = ({sliderWidth, min, max, step, onValueChange, startPos, endPos, shadowIos, shadowAndroid}) => {
  ...
  useEffect(() => {
    getLanguageData()
    console.log('shadowios', shadowIos)
    console.log('shadowandroid', shadowAndroid)
  }, [])
  ...
  return (
    <View style={[styles.sliderContainer, {width: sliderWidth}]}>
      ...
      <Animated.View style={[styles.thumbRightShadow, animatedStyle2, {shadowColor: `${shadowIos}`}]}>
      {Platform.OS === 'android' && <Shadow
                distance={80}
                offset={[-30, 30]}
                startColor={shadowAndroid}
              />}
      <Animated.View style={[styles.thumbRightShadow, {shadowColor: `${shadowIos}`}]}>
      <Animated.View style={[styles.thumb]}>
        <Animated.View style={[styles.label, {flexDirection: 'row', justifyContent: 'center', alignItems: 'flex-start', width: '100%'}]}>
          <Text style={styles.labelText}>
            {maxValue}
          </Text>
        </Animated.View>
