...
import { Text, View, FlatList, StyleSheet, TouchableOpacity, ActivityIndicator, ImageBackground,
  Platform, RefreshControl, Dimensions, Modal, TextInput} from 'react-native'
...
function Goods(props) {
  ...
  const {companie_item} = props.route.params
  ...
  const loadData = () => {
    ...
    setLoading(true)
    fetch(`${BASE_URL}/api/companiegoods/${id}/`, {
      method:"GET",
      headers: {
  ...
  const createGood = () => {
    ...
    fData.append('companie', id)
    fData.append('type', 'тип не определен')
    fetch(`${BASE_URL}/api/goods/`, {
      method:"POST",
      headers: {
        'Authorization': `${token._j}`,
  ...
  useEffect(() => {
    props.navigation.setOptions({
      title: language == 'English' ? companie_item.title : companie_item.title, headerTitleStyle: {fontFamily: 'SftBold', fontSize: 20},
      headerRight: () => (
        <TouchableOpacity
  ...
  const clickedItem = (id) => {
    props.navigation.navigate("Good", {id: id, data: id})
  }
      ...
      <Modal
        ...
        >
            ...
            alignItems: 'flex-end', paddingBottom: 11, flexDirection: 'row', width: '100%' }}>
                ...
                <View style={{ width: '60%' }}>
                    <Text style={{ color: '#fff', fontSize: 20, textAlign: 'center', fontWeight: '600', fontFamily: 'SftBold' }}>{language == 'English' ? 'Create good' : 'Добавить товар'}</Text>
                </View>
            </View>
            ...
            >
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() => {
                  fData != undefined ? createGood() : alert(language == 'English' ? 'Choose the photo!' : 'Выберите фото!')
                }}
              >
                <Text style={[{ color: '#fff', textAlign: 'center', fontSize: 20, fontWeight: '400',
                  marginLeft: 10, fontFamily: 'SftMedium' }, 
                ]}>{language == 'English' ? 'Create good' : 'Создать товар'}</Text>
              </TouchableOpacity>
            </KeyboardAwareScrollView>
            {loadingChange &&
